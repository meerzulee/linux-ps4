...
static u64 vram_size = 1024 * 1024;  /* 1 MB unit - value is now in MB */
...
static void configure_vram(void)
{
    ...
    WR32(mmio_base + CONFIG_MEMSIZE, vram_mb * vram_size >> 20);
}
...
static void setup_mtrr(void)
{
    ...
    wrmsr(MTRR_MASK(3), (0xffffffffff - vram_mb * vram_size + 1) | 0x800);
...
static void cpu_quiesce_gate(void *arg)
{
    ...
    *(volatile u64 *)PA_TO_DM(0xfc000018) &= ~1;
    ...
    kern.printf("\n");
    if (sb_id == SB_BAIKAL) {
        kern.printf("kexec: Detected Baikal Southbridge, disabling IOMMU (eth MSI SKIPPED for TX test)...\n");
         // Disable all MSIs on Baikal (bus=0, slot=20)
        disableMSI(0xf80a00e0); //func = 0 Baikal ACPI
        // disableMSI(0xf80a10e0); //func = 1 Baikal Ethernet Controller -- SKIPPED: leave MSI vectors unmasked so ps4_mts can receive TX-completion IRQs
