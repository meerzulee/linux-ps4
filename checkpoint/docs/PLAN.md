...
| **6.x with Option B v1–v5** (`0007-bpcie-option-b-msi-parent`) | 🛠 In progress 2026-05-09. Three-part fix: (a) mark bpcie domain as `IRQ_DOMAIN_FLAG_MSI_PARENT` with `msi_parent_ops`, (b) `dev_set_msi_domain()` install on each Baikal pdev (was missing — 5.4 worked via legacy fwnode lookup that 6.2 removed), (c) `bus_token = DOMAIN_BUS_AMDVI` to satisfy x86_vector's gating switch, (d) `MSI_FLAG_MULTI_PCI_MSI` in `supported_flags` so bpcie's own ICC subfunc alloc isn't capped at 1 vector. v1 (parent flag only) → kernel ignored. v2 (added install) → infinite recursion in our wrapper, hung at 4.6 s. v3 (kernel helper) → x86 WARN'd on bus_token. v4 (AMDVI) → bpcie probe failed -EIO. **v5 (multi-MSI) pending hardware test (this commit).** Caveat: per-device child uses standard `handle_edge_irq` not bpcie's subfunction demuxer — fine for xHCI single-vector but may need porting if SDHCI/AHCI/sky2 also need demuxing. See LEARNINGS.md "Linux 6.2 PCI MSI domain rework" for full diagnostic timeline. |
