/*
 * Orbis MTS BAR0 dumper (HDD variant) — payload for PSFree-Enhanced (FW 12.02).
 *
 ...
 *
 * Differences vs sibling orbis-bar-dumper (USB variant):
 *  - Writes to /data/orbis-dump/<FW>/mts-bar0.bin on internal HDD.
 *  - Skips wait_for_usb() — no USB device needed.
 *  - Output is fetched via FTP from /data afterwards (avoids FAT32
 *    fragmentation + USB throughput bottleneck).
 *
 * Safety chain (unchanged from USB variant):
 *   *(kbase + 0x021f4938)      -> global mts softc pointer slot
 *   *(softc + 0x3068)          -> BAR0 struct resource *
 *   *(bar0_res + 0x10)         -> BAR0 KVA / r_handle
 *
 * Each dereference is range-checked (bit-63 must be set for KVA) before
 * a bounded 16-byte volatile probe; only if the probe returns plausibly
 * valid bytes do we do the full 4 KiB copyout.
 */
...
int _main(struct thread *td) {
  ...
  printf_notification("Orbis MTS BAR0 dumper [HDD] (FW %s, kbase=0x%llx)",
  ...
  if (ok) {
    printf_notification("Done. Fetch via FTP: /data/orbis-dump/%s/mts-bar0.bin",
                        fw_version);
  } else {
    printf_notification(
